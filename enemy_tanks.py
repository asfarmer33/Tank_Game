...
class enemy_tank(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if pygame.time.get_ticks() - self.time_shot > 3000: # every 3 seconds the enemy tank can shoot
            self.bullet_group.add(Bullets(self.screen, self.x, self.y, self.angle, self.player_group))  # creates bullet that can hit the player
    ...
    def turn(self):
        ...
        if pygame.time.get_ticks() - self.time_turn > 10:
            print(f"{self.angle}: {self.player_angle()}")
            if abs(self.player_angle() - self.angle) > 0:
                if self.player_angle() - self.angle < 0:
                    self.angle -= self.turn_speed
                else:
                    self.angle += self.turn_speed
            self.time_turn = pygame.time.get_ticks()
